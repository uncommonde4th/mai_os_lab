...
void load_library() {
    if (impl == 0) {
        library = dlopen("./libimpl0.so", RTLD_LAZY);
    } else {
        library = dlopen("./libimpl1.so", RTLD_LAZY);
    }
    if (library == NULL) {
        write_str("Библиотека не найдена\n");
    }
    cos_derivative = (float(*)(float, float))dlsym(library, "cos_derivative");
    convert = (char*(*)(int))dlsym(library, "convert");
}
...
int main() {
    ...
    while (run) {
        ...
        write_str("\t0 - Переключить реализацию контрактов\n");
        write_str("\t-1 - Выход.\n");
        write_str("Команда: ");
